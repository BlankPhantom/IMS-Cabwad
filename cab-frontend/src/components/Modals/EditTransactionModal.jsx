...
const EditTransactionModal = ({
  ...
  const [showEditExistingProductModal, setShowEditExistingProductModal] =
    useState(false);
  ...
  const fetchProductsForTransaction = async (transactionDetailsID) => {
    try {
      ...
      const filteredProducts = data.filter(
        (product) => product.transactionDetailsID === transactionDetailsID
      );
  ...
  const handleAddProductSubmit = (e) => {
    ...
    const newProduct = {
      ...
      purchasedFromSupplier:
        parseInt(productData.purchasedFromSupplier, 10) || 0, // ✅ Convert to number
      returnToSupplier: parseInt(productData.returnToSupplier, 10) || 0,
      transferFromWarehouse:
        parseInt(productData.transferFromWarehouse, 10) || 0,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const transactionResponse = await fetch(
        API_ENDPOINTS.UPDATE_TRANSACTION(transactionData.transactionDetailsID),
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (localProducts.length > 0) {
        const productRequests = localProducts.map((productData) => {
          const productPayload = {
            ...
            purchasedFromSupp:
              parseInt(productData.purchasedFromSupplier, 10) || 0,
            returnToSupplier: parseInt(productData.returnToSupplier, 10) || 0,
            transferFromWH:
              parseInt(productData.transferFromWarehouse, 10) || 0,
  ...
  const handleSaveExistingProduct = async (e) => {
    ...
    try {
      ...
      const productPayload = {
        ...
        purchasedFromSupp:
          parseInt(editProductData.purchasedFromSupplier, 10) || 0,
        returnToSupplier: parseInt(editProductData.returnToSupplier, 10) || 0,
        transferFromWH:
          parseInt(editProductData.transferFromWarehouse, 10) || 0,
  ...
  return (
    <>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Select
                ...
                onChange={handleEditPurposeChange}>
            ...
            {transactionProducts.length > 0 ? (
              <ul className="mt-3">
                {transactionProducts.map((product) => (
                  <li key={product.transactionProductID}>
                    {product.itemName}{" "}
                    {/* ✅ Display product name instead of itemID */}
                    <span
                      style={{
                        color: "#ffcc00",
                        marginRight: "5px",
              ...
              <p className="text-muted">
                No products added to this transaction
              </p>
            )}
      ...
      <Modal
        show={showEditExistingProductModal}
        onHide={() => {
          setShowEditExistingProductModal(false);
          handleShow();
        }}
